...
/*
*purpose: destroys the remaining heap allocations to free memory
*input: pointer to address of matrix and unsigned integer - number of matrixes
*output:
*/
...
/*
*purpose:the execution of the program, creating matrixes and adding them to arrays, etc
*input:input is given from the command line (arguments are commands that have been coded)
*output:
*/
int main (int argc, char **argv) {
	...
	Matrix_t *temp = NULL;
	create_matrix (&temp,"temp_mat", 5, 5); // TODO ERROR CHECK - done in matrix.c
	add_matrix_to_array(mats,temp, 10); //TODO ERROR CHECK NEEDED - done in matrix.c
	...
	write_matrix("temp_mat", mats[mat_idx]); // TODO ERROR CHECK- done in matrix.c
	...
	//TODO FUNCTION COMMENT
/*
*purpose: run the commands of the program
*input:pointer to address of commands, pointer to address of matrix_t, and unsigned integer for number of matrices
*output:
*/
void run_commands (Commands_t* cmd, Matrix_t** mats, unsigned int num_mats) {
	...
	if (strncmp(cmd->cmds[0],"display",strlen("display") + 1) == 0
		...
		&& cmd->num_cmds == 4) {
			...
			if (mat1_idx >= 0 && mat2_idx >= 0) {
				...
				add_matrix_to_array(mats,c, num_mats); //TODO ERROR CHECK NEEDED - done in matrix.c
		...
		if (mat1_idx >= 0 ) {
				...
				duplicate_matrix (mats[mat1_idx], dup_mat); //TODO ERROR CHECK NEEDED - done in matrix.c
				add_matrix_to_array(mats,dup_mat,num_mats); //TODO ERROR CHECK NEEDED - done in matrix.c
		...
		if (mat1_idx >= 0 ) {
			bitwise_shift_matrix(mats[mat1_idx],cmd->cmds[2][0], shift_value); //TODO ERROR CHECK NEEDED - done in matrix.c
		...
		add_matrix_to_array(mats,new_matrix, num_mats); //TODO ERROR CHECK NEEDED - done in matrix.c
		...
		create_matrix(&new_mat,cmd->cmds[1],rows, cols); //TODO ERROR CHECK NEEDED - done in matrix.c
		add_matrix_to_array(mats,new_mat,num_mats); // TODO ERROR CHECK NEEDED - done in matrix.c
		...
		random_matrix(mats[mat1_idx],start_range, end_range); //TODO ERROR CHECK NEEDED - done in matrix.c
	...
	//TODO FUNCTION COMMENT
/*
*Purpose: finds a matrix with a given name
*input: an address to a pointer of type matrix_t, and an unsigned integer for the number of matrixes and a pointer to a constant target
*output: returns the index of the matrix, otherwise it returns -1
*/
